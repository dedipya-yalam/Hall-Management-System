...
from databases.hall_clerk_login_db import hall_clerk_login_db

from databases.student_db import student_db
from databases.hall_db import hall_db
from databases.complaints_db import complaints_db
from databases.hall_workers_db import hall_workers_db
...
root = Tk()
root.title("Hall Management System")
...
class home_page:
	...
	def pack_homepage(cls):
		...
		cls.title=Label(cls.homepage,text="Hall Management System, IIT Kharagpur",font=("times new roman",22,"bold","italic"),bg="light grey")
...
class student_login:
	...
	def sends_complaint(cls):
		...
		hal_name=cls.user_data[0][7]
		cls.hall_list=hall_db.get_record_by_name(hal_name)
		war_nam=cls.hall_list[0][9]
		cls.war_list=warden_db.get_records_by_name(war_nam)
		war_id=cls.war_list[0][0]
		complaints_db.insert_records((stu_id,stu_name,war_id,war_nam,hal_name,com," "))
	...
	def profile(cls):
		...
		pdf.set_xy(10.5, 10)
		file = os.path.join(dirn,'images/IIT_Kharagpur_Logo.png')
		pdf.image(file, w = 30, h = 30)

		pdf.set_xy(20.0,8.0)
		pdf.set_font('Arial', 'B', 15)
		pdf.set_text_color(0, 0, 139)
		pdf.cell(w=210.0, h=40.0, align='C', txt="INDIAN INSTITUTE OF TECHNOLOGY KHARAGPUR", border=0)
		...
		pdf.cell(w=210.0, h=40.0, align='L', txt="Hall Name:", border=0)
...
class HMCChairman_login:
	...
	def hmc_main(cls):
		...
		cls.Administer_hall=Button(cls.hmc_menu, text="Administer halls",bg="Pale Turquoise", width=20,borderwidth=0,command=cls.administer_halls).grid(row=0,column=1,padx=10)
		cls.Allocate_hall=Button(cls.hmc_menu, text="Allocate hall",bg="Pale Turquoise", width=20,borderwidth=0, command=cls.allocate_halls).grid(row=0,column=2,padx=10)
	...
	@classmethod
	def administer_halls(cls):
		cls.hmc_administer_halls = Frame(cls.HMC_main, borderwidth=4, bg="black")
		cls.hmc_administer_halls.place(x=-3,y=28,width=1050,height=600)

		file = os.path.join(dirn,'images/wood_background.jpg')
		cls.image = Image.open(file)
		cls.image = cls.image.resize((1050,600))
		cls.photo = ImageTk.PhotoImage(cls.image)
		cls.label = ttk.Label(cls.hmc_administer_halls, image = cls.photo)
		cls.label.place(x=-3,y=-3,width=1048,height=600)

		cls.hmc_room = Frame(cls.hmc_administer_halls, borderwidth=4, bg="peach puff")
		...
		cls.hallnameLabel = Label(cls.hmc_room, text="Hall Name",bg="peach puff").place(x=5, y=55)
		cls.hallname=StringVar()
		cls.hallnameEntry = Entry(cls.hmc_room, textvariable=cls.hallname).place(x=200, y=55) 

		list=["Type","New","Old"]
		cls.halltypelabel = Label(cls.hmc_room, text="Hall Type",bg="peach puff").place(x=5,y=85)
		cls.halltypeEntry=ttk.Combobox(cls.hmc_room,values=list,width=17)
		cls.halltypeEntry.current(0)
		cls.halltypeEntry.place(x=200,y=85) 
		cls.halltypeEntry.bind("<<ComboboxSelected>>",cls.passs)

		cls.hallamenity_chargesLabel = Label(cls.hmc_room, text="Amenities charge",bg="peach puff").place(x=5, y=115)
		cls.hallamenity_charges=StringVar()
		cls.hallamenity_chargesEntry = Entry(cls.hmc_room, textvariable=cls.hallamenity_charges).place(x=200, y=115) 
		...
		cls.hmc_work = Frame(cls.hmc_administer_halls, borderwidth=4, bg="peach puff")
		...
		cls.Set_up_hall=Button(cls.hmc_administer_halls, text="Setup/update hall",bg="Pale Turquoise", width=20,borderwidth=0, command=cls.setup_halls).place(x=460,y=530)
		cls.Cancel=Button(cls.hmc_administer_halls, text="Cancel",bg="Pale Turquoise", width=20,borderwidth=0, command=cls.cancel).place(x=460,y=560)


	@classmethod
	def setup_halls(cls):
		# if(cls.halltypeEntry.get()=="Old")
		hal_list=hall_db.get_all_records()
		halname_list=[]
		for i in hal_list:
			halname_list.append(i[1])
		if(cls.hallname.get() not in halname_list):
			hall_db.insert_record((cls.hallname.get(),cls.halltypeEntry.get(),cls.singlerc.get(),cls.doublerc.get(),0,0,cls.singlerr.get(),cls.doublerr.get(),cls.wardenname.get(),cls.MessManagername.get(),cls.clerkname.get(),cls.hallamenity_charges.get()))
			li=hall_db.get_record_by_name(cls.hallname.get())
			warden_db.insert_record((cls.wardenname.get(),cls.wardenname.get(),cls.wardenpassword.get(),"",cls.wardensalary.get(),cls.hallname.get(),li[0][0],cls.clerksalary.get(),0,0,0,0,0))
			mess_manager_login_db.insert_record((cls.MessManagername.get(),cls.MessManagerPassword.get(),li[0][0]))
			hall_clerk_login_db.insert_record((cls.clerkname.get(),cls.clerkpassword.get(),li[0][0]))
	...
	@classmethod
	def allocate_halls(cls):
		cls.hmc_allocate_hall = Frame(cls.HMC_main, borderwidth=4, bg="black")
		cls.hmc_allocate_hall.place(x=-3,y=28,width=1050,height=600)

		file = os.path.join(dirn,'images/wood_background.jpg')
		cls.image = Image.open(file)
		cls.image = cls.image.resize((1050,600))
		cls.photo = ImageTk.PhotoImage(cls.image)
		cls.label = ttk.Label(cls.hmc_allocate_hall, image = cls.photo)
		cls.label.place(x=-3,y=-3,width=1048,height=600)
		...
		allocate_halls_tree = ttk.Treeview(cls.label)

		#make treeview table
		allocate_halls_tree['columns'] = ("ID","Name")
		allocate_halls_tree.column("#0", width = 0,stretch = NO)
		allocate_halls_tree.column("ID",anchor = W, width = 100)
		allocate_halls_tree.column("Name", anchor = W,width = 200)

		allocate_halls_tree.heading("#0", text = "", anchor = W)
		allocate_halls_tree.heading("ID", text = "ID", anchor = W)
		allocate_halls_tree.heading("Name", text = "Name", anchor = W)
		...
		count=0
		for value in li:        
			allocate_halls_tree.insert(parent='',index='end',iid=count,text='Parent',values=value)
			count=count+1

		cls.but6=Button(cls.label,text="Allocate hall",bg="Pale Turquoise", width=20,borderwidth=0,command=cls.allocate)
		cls.but6.place(x=750,y=250)

		allocate_halls_tree.place(x=250,y=50)
	...
	def allocate(cls):
		...
		for beta in get_all_students:
			if beta[7] == 'not allocated':
				student_db.update_hall_name('LBS',beta[0])
...
class clerk_login:
	...
	#correct aythe clerk page open
		...
		# cls.clelist = hall_clerk_login_db.get_record_by_username(cls.username.get())
...
class MessManager_login:
	...
	def update_mess_fee(cls):
		...
		count=0
		for i in stu_list :         
			listhall=hall_db.get_record_by_name(i[7])
			print(listhall)
			if  listhall:
				k=0
				for j in (listhall[0]):
